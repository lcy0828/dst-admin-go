...
func shardProcessFromArguments(pid int32, name string, arguments []string) shared.ShardProcessReport {
	item := shared.ShardProcessReport{PID: pid, Executable: name}
	item.Cluster = commandFlag(arguments, "-cluster")
	item.Shard = commandFlag(arguments, "-shard")
	item.StorageRoot = commandFlag(arguments, "-persistent_storage_root")
	item.ConfigDirectory = commandFlag(arguments, "-conf_dir")
	return item
}
