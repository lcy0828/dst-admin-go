...
// MonitorServerStatus 监控服务器状态并管理玩家信息定时任务
// 每30秒检测一次服务器是否正在运行
// 如果有服务器运行，确保玩家信息定时任务启用
// 如果没有服务器运行，禁用玩家信息定时任务
func MonitorServerStatus() (string, error) {
	...
	for i := range tasks {
		task := &tasks[i]
		if task.Type == "function" && task.Target == "read_player_config" {
			// 获取任务参数（存档名和世界名）
	...
	// 如果没有运行中的服务器，禁用所有玩家配置任务
	if len(runningServers) == 0 {
		log.Printf("[ServerMonitor] 没有运行中的服务器，禁用所有玩家配置任务")
		for _, task := range playerConfigTasks {
			if task.Status == 1 { // 如果任务当前是启用状态
				if err := models.UpdateTaskStatus(task.ID, 0); err != nil {
					log.Printf("[ServerMonitor] 禁用任务失败 (ID: %d): %v", task.ID, err)
				} else {
					log.Printf("[ServerMonitor] 已禁用任务: %s (ID: %d)", task.Name, task.ID)
				}
			}
		}
		return "没有运行中的服务器，已禁用所有玩家配置任务", nil
	}
			...
			if task, exists := playerConfigTasks[sessionName]; exists {
				...
				if task.Status == 0 {
					if err := models.UpdateTaskStatus(task.ID, 1); err != nil {
						log.Printf("[ServerMonitor] 启用任务失败 (ID: %d): %v", task.ID, err)
					} else {
						log.Printf("[ServerMonitor] 已启用任务: %s (ID: %d)", task.Name, task.ID)
					}
				}
			} else {
				// 如果任务不存在，创建新任务
				...
				if err := newTask.SetArgs([]interface{}{archiveName, worldName}); err != nil {
					log.Printf("[ServerMonitor] 设置任务参数失败: %v", err)
					continue
				}

				// 添加任务
				if err := taskManager.AddTask(newTask); err != nil {
					log.Printf("[ServerMonitor] 创建任务失败: %v", err)
				} else {
					log.Printf("[ServerMonitor] 创建任务成功: %s", taskName)
				}
			}
		}
	}
	...
	return fmt.Sprintf("已检测到 %d 个运行中的服务器，并相应地管理了玩家配置任务", len(runningServers)), nil
}
...
func RegisterServerMonitorTasks(manager *TaskManager) {
	...
	manager.RegisterFunctionWithInfo("monitor_server_status", MonitorServerStatus, FunctionInfo{
		...
		Description: "监控服务器状态并管理玩家信息定时任务",
