...
同一节点允许运行多个房间和多个世界分片，但产品默认按“每个运行中的世界分片预留 1 个 CPU 核心”计算建议容量：
...
已交付说明：支持同一节点规划多个房间和多个世界分片；容量按运行 Shard 总数而非房间数计算，默认每个 Shard 占用 1 个物理核心预算并额外预留 1 核。超出建议值只要求用户确认并告警，不构成硬限制或性能保证。
