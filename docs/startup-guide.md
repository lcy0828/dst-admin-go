...
2. 等待本机世界和后台任务停止后保存，角色会在当前进程中生效，无需重启管理服务。
...
“加入管理中心”，填写上级 `ws(s)://…/agent` 和节点连接密钥，等待本机世界和后台任务停止后保存应用。
...
通过页面保存角色、目录和运行工具设置时会自动应用，无需重启管理服务。切换前需停止本机世界并等待后台任务完成；应用失败会回退配置。修改目录不会搬动、导入或删除存档。环境变量锁定的设置仍需修改部署配置并重启。
...
| Agent 不在线 | 检查控制端角色是否已应用、`/agent` 地址、双方密钥、环境变量覆盖和反向代理 WebSocket |
