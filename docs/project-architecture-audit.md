...
| 已完成 | 存档导入前端闭环 | 安全分析、任务状态、应用和恢复可见 | L | 保持回归 |
| 已完成 | 世界创建/删除、房间删除、SMTP 测试前端闭环 | 补齐本地服完整生命周期和部署验证 | M | 保持回归 |
...
| 已完成（Phase 1-9） | 多节点 Agent 集中管理 | 一个房间跨节点运行、统一控制、观测、备份、Mod 与版本发布 | XL | 保持本地默认、远程显式配置 |
...
> 落地状态：Runtime 2.3.1 已完成自动化实现，包含 A/B Telemetry、空服暂停短刷新、固定允许列表短动作与结构化回执、世界事件、有界诊断、热加载清理、世界状态快照和旧控制台 fallback。Debian 12 真实 DST 空服暂停验收已通过；macOS、满员、全部短动作及代表性 Mod 组合仍是发布前门禁。
...
> 落地状态：前端已有 200 余条 Node 源码/协议回归测试，并通过 lint 与生产构建；当前仓库没有 Playwright/browser E2E，真实浏览器主流程仍是明确发布缺口，不能用历史测试描述替代。
...
- `ui.language` 已接入中文和英文目录及 fallback；新增正式页面必须同时补齐两套静态 key，协议值和用户内容不能被翻译改写。
...
批次 1、本地存档导入与房间/世界生命周期、SMTP 分阶段诊断，以及 Runtime 2.3.1 玩家/事件/诊断/世界状态链路都已实现；Runtime 继续保留受审计的控制台 fallback，Capability Manifest、领域标签 E2E 和可测量 bundle 门禁也已进入 CI。Debian 空服暂停链路已实机通过；当前下一阶段应补真实 macOS、满员与代表性 Mod、存档导入和 SMTP 提供商验收；日志与 Mod 锁优化仍必须先测量，视觉收敛必须跟随真实功能页面。
