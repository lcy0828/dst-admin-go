...
- Go 消费的受管 JSON 输出统一使用 `json.encode_compliant`。DST 的 `json.encode` 面向游戏存档，会把名字或错误信息中的单引号写为非法 JSON 转义 `\'`，导致整份快照读取失败。此修复保持 `2.4.6` 协议兼容，安装清单通过资产 SHA-256 识别更新，现有分片可备份后热加载。
...
- `pause_when_empty=true` 造成模拟暂停时，DST 的普通和 static scheduler 都不会继续周期任务。玩家定时采集复用 Runtime 的暂停观察：确认暂停的分片保留上次数据和采集时间，不发送 `DSTAdmin.Refresh()` 或控制台 fallback；全部分片暂停时跳过任务，恢复运行后继续采集。手动刷新仍可在快照过期后发送短入口，等待玩家、世界和 Health sequence 同时前进；暂停状态未知时保持原有采集逻辑。
- 玩家定时任务跳过采集前仍处理进程状态：确认停止或失败的分片将旧在线玩家标为离线；Agent 失联、启动中或状态未知时仅将旧记录标为未确认，不推断离线。暂停分片的玩家记录也按上次观察展示，避免错把未观察到的重启前数据当成当前在线状态。该处理无需游戏命令，不推进玩家采集时间，状态不变时不重复写入；管理服务重启后也可从已有数据库记录恢复。
