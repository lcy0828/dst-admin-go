...
// GetPositionManager 获取全局位置管理器
func GetPositionManager() *PositionManager {
	positionManagerOnce.Do(func() {
		// 确定位置记录文件路径
		positionsFile := filepath.Join(dstSavePath, "watcher_positions.json")

		// 创建位置管理器
		manager, err := NewPositionManager(positionsFile)
		if err != nil {
			log.Printf("[PositionManager] 创建位置管理器失败: %v，将创建新的位置管理器", err)
...
// ShutdownPositionManager 关闭全局位置管理器
func ShutdownPositionManager() {
